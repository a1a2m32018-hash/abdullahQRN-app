...
function IconButton({ label, onClick, children, active = false, testId }: { label: string; onClick: () => void; children: ReactNode; active?: boolean; testId: string }) {
  return (
    <button aria-label={label} data-testid={testId} onClick={onClick} className={`group relative flex h-11 w-11 items-center justify-center rounded-full border transition-all duration-200 ${active ? 'border-[hsl(var(--accent))] bg-[hsl(var(--accent)/.18)] text-[hsl(var(--primary))]' : 'border-[hsl(var(--border))] bg-[hsl(var(--card)/.72)] text-[hsl(var(--muted-foreground))] hover:-translate-y-0.5 hover:border-[hsl(var(--accent))] hover:text-[hsl(var(--primary))]'}`}>
      {children}
...
function PageContent({ page, isBookmarked, onBookmark }: { page: NonNullable<ReturnType<typeof useQuranPage>['page']>; isBookmarked: boolean; onBookmark: () => void }) {
...
function PageArrow({ side, disabled, onClick }: { side: 'next' | 'previous'; disabled: boolean; onClick: () => void }) {
  const isNext = side === 'next';
  return <button data-testid={`button-fixed-${side}-page`} aria-label={isNext ? 'الصفحة التالية' : 'الصفحة السابقة'} title={isNext ? 'الصفحة التالية' : 'الصفحة السابقة'} onClick={onClick} disabled={disabled} className={`fixed ${isNext ? 'left-2 sm:left-5' : 'right-2 sm:right-5'} top-1/2 z-30 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-[hsl(var(--accent)/.6)] bg-[hsl(var(--card)/.9)] text-[hsl(var(--primary))] shadow-[0_5px_18px_hsl(var(--foreground)/.1)] backdrop-blur-sm transition-all hover:scale-105 hover:bg-[hsl(var(--accent)/.18)] disabled:cursor-not-allowed disabled:opacity-30 sm:h-12 sm:w-12`}><span className="sr-only">{isNext ? 'الصفحة التالية' : 'الصفحة السابقة'}</span>{isNext ? <ChevronLeft size={23} strokeWidth={1.7} /> : <ChevronRight size={23} strokeWidth={1.7} />}</button>;
}
