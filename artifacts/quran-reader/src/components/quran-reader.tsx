...
function PageContent({ page, isBookmarked, onBookmark }: { page: NonNullable<ReturnType<typeof useQuranPage>['page']>; isBookmarked: boolean; onBookmark: () => void }) {
  ...
  return <div className="page-in px-5 pb-10 pt-7 sm:px-10 sm:pb-12 sm:pt-9" dir="rtl"><div className="mb-6 flex items-center justify-between gap-4"><div className="flex items-center gap-2 text-[hsl(var(--muted-foreground))]"><span className="h-px w-8 bg-[hsl(var(--accent)/.6)]" /><span className="text-[10px]">الجزء {toArabicNumber(page.ayahs[0]?.juz ?? 1)}</span></div><button data-testid="button-bookmark-current" aria-label={isBookmarked ? 'إزالة حفظ الصفحة' : 'حفظ الصفحة'} onClick={onBookmark} className={`flex items-center gap-2 rounded-full border px-3 py-2 text-[10px] transition-colors ${isBookmarked ? 'border-[hsl(var(--accent))] bg-[hsl(var(--accent)/.15)] text-[hsl(var(--primary))]' : 'border-[hsl(var(--border))] text-[hsl(var(--muted-foreground))] hover:border-[hsl(var(--accent))]'}`}>{isBookmarked ? <BookmarkCheck size={15} /> : <Bookmark size={15} />}{isBookmarked ? 'محفوظ' : 'حفظ الموضع'}</button></div>{firstSurah && <div className="mb-8 text-center"><div className="mx-auto mb-3 flex items-center justify-center gap-3 text-[hsl(var(--accent))]"><span className="h-px w-12 bg-[hsl(var(--accent)/.5)]" /><span className="text-lg">۞</span><span className="h-px w-12 bg-[hsl(var(--accent)/.5)]" /></div><h1 className="font-serif text-3xl text-[hsl(var(--primary))] sm:text-4xl">{firstSurah.name}</h1></div>}<div className="font-serif text-[1.68rem] leading-[2.38] text-[hsl(var(--foreground))] sm:text-[2rem] sm:leading-[2.48]">{page.ayahs.map((ayah, index) => <span key={ayah.number} data-testid={`text-ayah-${ayah.number}`} className={surahBreaks.includes(index) && index !== 0 ? 'inline-block pt-7' : ''}>{ayah.text}<span className="mx-1 inline-flex h-7 w-7 translate-y-[-1px] items-center justify-center rounded-full border border-[hsl(var(--accent)/.72)] align-middle text-[.62em] text-[hsl(var(--primary))]">{toArabicNumber(ayah.numberInSurah)}</span>{' '}</span>)}</div></div>;
}
...
export default function QuranReader() {
  ...
  return <main className="paper-grain min-h-[100dvh] bg-[hsl(var(--background))]" dir="rtl">
    ...
    <section className="mx-auto max-w-5xl px-3 pb-24 pt-5 sm:px-7 sm:pt-8">
      ...
      <div className="mushaf-frame mx-auto max-w-3xl overflow-hidden rounded-[1.15rem]" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} onClick={onPageTap}>
