...
function PageArrow({ side, disabled, onClick }: { side: 'right' | 'left'; disabled: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`fixed ${isRight ? 'right-2 sm:right-6' : 'left-2 sm:left-6'} top-1/2 z-30 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-[hsl(var(--accent)/.25)] bg-[hsl(var(--card)/.12)] text-[hsl(var(--primary))] shadow-[0_4px_12px_hsl(var(--foreground)/.04)] backdrop-blur-[1px] transition-all hover:scale-105 hover:bg-[hsl(var(--accent)/.2)] hover:border-[hsl(var(--accent)/.5)] active:scale-95 disabled:cursor-not-allowed disabled:opacity-10 sm:h-12 sm:w-12`}
    >
